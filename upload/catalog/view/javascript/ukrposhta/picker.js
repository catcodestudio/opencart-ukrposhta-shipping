(() => {
  ...
  const matchZone = (zoneSel, area) => {
    ...
    // Zone names may be Cyrillic (uk-ua store language): transliterate them too,
    // and never let an empty normalised name match everything (it picked the
    // first zone — «Автономна Республіка Крим»).
  ...
  const showOwnNative = () => {
    document.querySelectorAll('.up-native-hidden').forEach((n) => n.classList.remove('up-native-hidden'));
  };
  const hideNativeAddress = () => {
    if (intlMode) return;
    // Only while the Ukrposhta method is the chosen one.
  ...
  /**
   * Re-price the order for the branch that was just chosen.
   *
   * The widget only opens AFTER the carrier is selected, so the quote OpenCart
   * saved at that moment was calculated without a recipient index — i.e. always
   * the flat fallback from the settings, never the live Ukrposhta tariff. The
   * controller drops the cached quote list in setSelection; asking for it again
   * and re-saving the same carrier is what moves the ORDER total, because the
  ...
  const requoteAfterPick = () => {
    ...
    // its own serial queue. A second, parallel save from here interleaved with
    // it: shipping_method.save dropped the payment method and the list came back
    // empty.
    if (document.getElementById('cc-op')) return;
    const shipCode = selectedShippingCode();
    if (shipCode.indexOf('ukrposhta.') !== 0) return;
    const $ = window.jQuery;
  ...
  // the save — the customer's radio wins, otherwise the widget waited for a
  // code that never came when that save was queued or refused.
  const selectedShippingCode = () =>
    (document.querySelector('input[name="cc_shipping_method"]:checked')?.value
      ...
      || '');
  let upOwned = false;
  // The native address form is ours to hide and fill only while Ukrposhta is
  // the chosen method; any other method gets it back, visible and with the
  // customer's own values.
  const ownNative = (isUp) => {
    const code = selectedShippingCode();
    if (isUp) {
      hideNativeAddress();
      if (!upOwned) { upOwned = true; seedNative(); fillNativeAddress(); }
    } else {
      upOwned = false;
      showOwnNative();
  ...
  const wireGate = () => {
    ...
    // One Page redraws its radio list after every quote — re-gate on each redraw.
  ...
  const seedNative = () => {
    // Placeholders only while Ukrposhta is the chosen method (and not abroad).
    if (selectedShippingCode().indexOf('ukrposhta.') !== 0 || intlMode) return;
    ccAddr.claim();
    const country = q1(NATIVE.country);
    if (country && country.value !== '220') {
      const opt = [...country.options].find((o) => /Україна|Ukraine/i.test(o.text));
  ...
  // On CatCode One Page Checkout start after its jQuery-ready handler, which
  // puts every same-origin request into one serial queue. Started earlier, our
  // first requests ran in parallel with its first shipping quote; OpenCart
  // writes the whole session back when a request ends, so the quote list could
  // be lost and shipping_method.save answered «Shipping method required!».
  const start = () => {
